...
func CheckUpdate(version string) string {
	...
	if versionCompare(version, latest) {
		logger.Infoln("当前有更新的 xxqg-automate 可供更新, 请前往 https://github.com/yockii/mybricks-publish/releases 下载.")
...
func lastVersion() (string, error) {
	response, err := http.Get("https://api.github.com/repos/yockii/mybricks-publish/releases/latest")
	if err != nil {
		return "", err
	}
